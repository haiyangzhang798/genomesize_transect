...
plotBeta_intercept = function(hM, post, param = "Support", plotTree = FALSE,
                    ...
                    smallplot=NULL, bigplot=NULL,newplot=TRUE)
{
  ...
  for (i in 1:hM$nc) {
    ...
    if (covNamesNumbers[2]) {
      covNames[i] = paste(covNames[i], sep = sep)
    }
  }
  ...
  else if(covOrder=="Original"){covorder=1:(hM$nc-1)} # modified by haiyang
  ...
  if(param=="Sign"){
    ...
    betaMat <- betaMat[-1,] # modified by haiyang
  }
  else if(param=="Mean"){
    toPlot = mbeta
    toPlot = toPlot * ((betaP>supportLevel) + (betaP<(1-supportLevel))>0)
  ...
  covNames <- covNames[-1] # modified by haiyang
